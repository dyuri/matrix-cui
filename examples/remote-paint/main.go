package main

import (
	"fmt"
	"os"

	"github.com/charmbracelet/lipgloss"
	matrixcui "github.com/dyuri/matrix-cui"
	"github.com/dyuri/matrix-cui/client"
)

// Color palette for painting
var colors = []lipgloss.Color{
	lipgloss.Color("#FF0000"), // 1: Red
	lipgloss.Color("#00FF00"), // 2: Green
	lipgloss.Color("#0000FF"), // 3: Blue
	lipgloss.Color("#FFFF00"), // 4: Yellow
	lipgloss.Color("#FF00FF"), // 5: Magenta
	lipgloss.Color("#00FFFF"), // 6: Cyan
	lipgloss.Color("#FFFFFF"), // 7: White
	lipgloss.Color("#FFA500"), // 8: Orange
	lipgloss.Color("#800080"), // 9: Purple
}

const socketPath = "unix:///tmp/matrix-cui.sock"

func main() {
	// Connect to remote matrix server
	m, err := client.NewMatrixRemote(socketPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to connect to server: %v\n", err)
		fmt.Fprintf(os.Stderr, "Make sure the matrix-server is running: cd examples/matrix-server && go run main.go\n")
		os.Exit(1)
	}
	defer m.Close()

	// Get initial dimensions
	width, height := m.Width(), m.Height()

	// Initialize with black background
	clearCanvas(m, width, height)

	// Subscribe to events
	if err := m.Subscribe([]string{"key", "mouse", "resize"}); err != nil {
		fmt.Fprintf(os.Stderr, "Failed to subscribe to events: %v\n", err)
		os.Exit(1)
	}
	defer m.Unsubscribe()

	// Get event channel
	eventChan := m.EventChannel()

	// Current drawing state
	currentColorIdx := 0
	isPainting := false

	fmt.Fprintf(os.Stderr, "Connected to server. Use mouse to paint, 1-9 to change color, C to clear, Q to quit.\n")

	// Main event loop
	for {
		select {
		case event := <-eventChan:
			// Handle mouse events
			if mouseEvent, ok := event.(matrixcui.MouseEvent); ok {
				switch mouseEvent.Action {
				case matrixcui.MouseActionPress:
					if mouseEvent.Button == matrixcui.MouseButtonLeft {
						isPainting = true
						paintCell(m, mouseEvent.X, mouseEvent.Y, colors[currentColorIdx])
					}
				case matrixcui.MouseActionRelease:
					if mouseEvent.Button == matrixcui.MouseButtonLeft {
						isPainting = false
					}
				case matrixcui.MouseActionMove:
					if isPainting {
						paintCell(m, mouseEvent.X, mouseEvent.Y, colors[currentColorIdx])
					}
				}
			}

			// Handle keyboard events
			if keyEvent, ok := event.(matrixcui.KeyEvent); ok {
				// Handle character keys
				if keyEvent.Key == matrixcui.KeyNone {
					switch keyEvent.Rune {
					case 'q', 'Q':
						return
					case 'c', 'C':
						// Clear canvas
						clearCanvas(m, width, height)
					case '1', '2', '3', '4', '5', '6', '7', '8', '9':
						// Change color
						colorNum := int(keyEvent.Rune - '1')
						if colorNum >= 0 && colorNum < len(colors) {
							currentColorIdx = colorNum
						}
					}
				}
			}

			// Handle resize events
			if resizeEvent, ok := event.(matrixcui.ResizeEvent); ok {
				m.Resize(resizeEvent.Width, resizeEvent.Height)
				width, height = resizeEvent.Width, resizeEvent.Height
				clearCanvas(m, width, height)
			}

			// Draw status bar
			drawStatusBar(m, width, height, currentColorIdx, colors)
		}
	}
}

func clearCanvas(m *client.RemoteMatrix, width, height int) {
	// Fill with black background
	emptyCell := matrixcui.NewCell(' ', lipgloss.Color(""), lipgloss.Color("#000000"))
	m.Fill(emptyCell)
}

func paintCell(m *client.RemoteMatrix, x, y int, color lipgloss.Color) {
	// Paint with a colored block character
	cell := matrixcui.NewCell('█', color, lipgloss.Color("#000000"))
	m.Put(x, y, cell)
}

func drawStatusBar(m *client.RemoteMatrix, width, height, colorIdx int, colors []lipgloss.Color) {
	// Don't draw if there's no room
	if height < 1 {
		return
	}

	// Clear status bar area (top line)
	statusCell := matrixcui.NewCell(' ', lipgloss.Color("#FFFFFF"), lipgloss.Color("#222222"))
	for x := 0; x < width; x++ {
		m.Put(x, 0, statusCell)
	}

	// Draw title
	title := "Remote Paint (Client)"
	titleCell := matrixcui.NewCell(' ', lipgloss.Color("#FFFFFF"), lipgloss.Color("#222222")).AddStyle(matrixcui.StyleBold)
	m.PutString(2, 0, title, titleCell)

	// Draw instructions
	instructions := "1-9:Color | C:Clear | Q:Quit | Click+Drag:Draw"
	instrCell := matrixcui.NewCell(' ', lipgloss.Color("#AAAAAA"), lipgloss.Color("#222222"))
	instrX := width - len(instructions) - 2
	if instrX > len(title)+4 {
		m.PutString(instrX, 0, instructions, instrCell)
	}

	// Draw color palette preview
	if height >= 3 {
		paletteY := 1
		paletteX := 2

		// Label
		labelCell := matrixcui.NewCell(' ', lipgloss.Color("#FFFFFF"), lipgloss.Color("#000000"))
		m.PutString(paletteX, paletteY, "Colors: ", labelCell)
		paletteX += 8

		// Draw each color swatch
		for i, color := range colors {
			// Current color indicator
			char := '█'
			if i == colorIdx {
				// Highlight current color with brackets
				bracketCell := matrixcui.NewCell('[', lipgloss.Color("#FFFF00"), lipgloss.Color("#000000"))
				m.Put(paletteX, paletteY, bracketCell)
				paletteX++
			}

			// Color swatch
			colorCell := matrixcui.NewCell(char, color, lipgloss.Color("#000000"))
			m.Put(paletteX, paletteY, colorCell)
			paletteX++

			if i == colorIdx {
				bracketCell := matrixcui.NewCell(']', lipgloss.Color("#FFFF00"), lipgloss.Color("#000000"))
				m.Put(paletteX, paletteY, bracketCell)
				paletteX++
			}

			// Space between colors
			m.Put(paletteX, paletteY, labelCell)
			paletteX++
		}
	}
}
